	...
	function getValue() {
		...
		// Блок: Количество уровней
			...
			if(levelText == '1 ярус'){
				...
				$('.shape.shapeLevel2').css('display','none');
				// Начинка 2 ярус
				...
				// Форма для второго яруса
				$('.shape.shapeLevel2').css('display','block');
				// Начинка 2 ярус
		...
		// Блок: Форма торта
			...
			let shapeNoneOne ='1 ярус';
			let shapeNoneTwo ='2 ярус';
			let cycle = 'Круг';
			let heart = 'Сердце';
			...
			let shapeValue = parseInt($('select[name=shapeLevel1] option:selected', 'form#calcForm').val());
			let shapeText = $('select[name=shapeLevel1] option:selected', 'form#calcForm').text();

			if(shapeText == shapeNoneOne){
				$('img#_imageOneShape').attr('src', 'images/shape/none.jpg');
			}else if(shapeText == cycle) {
				$('img#_imageOneShape').attr('src', 'images/shape/cycle.jpg');
			}else if(shapeText == heart) {
				$('img#_imageOneShape').attr('src', 'images/shape/heart.jpg');
			}else if(shapeText == rectangle) {
				$('img#_imageOneShape').attr('src', 'images/shape/rectangle.jpg');
			}else if(shapeText == square) {
				$('img#_imageOneShape').attr('src', 'images/shape/square.jpg');
			}

			// Получаем данные с select для 2 яруса
			let shapeTwoValue = parseInt($('select[name=shapeLevel2] option:selected', 'form#calcForm').val());
			let shapeTwoText = $('select[name=shapeLevel2] option:selected', 'form#calcForm').text();

			if(shapeTwoText == shapeNoneTwo){
				$('img#_imageTwoShape').attr('src', 'images/shape/none.jpg');
			}else if(shapeTwoText == cycle) {
				$('img#_imageTwoShape').attr('src', 'images/shape/cycle.jpg');
			}else if(shapeTwoText == heart) {
				$('img#_imageTwoShape').attr('src', 'images/shape/heart.jpg');
			}else if(shapeTwoText == rectangle) {
				$('img#_imageTwoShape').attr('src', 'images/shape/rectangle.jpg');
			}else if(shapeTwoText == square) {
				$('img#_imageTwoShape').attr('src', 'images/shape/square.jpg');
			}

		// Блок: Вес торта
			
			let valueWeight = $('input[name=weightCake1]', 'form#calcForm').val();
			valueWeight = 1500 * valueWeight;
		...
		// Получаем значение с select, а затем ищем имя option в двух массивах, чтобы вывести описание и изображение начинки
			...
			if(fillingText){
				$('.fillingOneCake > .infoOneFilling > p').html(fillingDescription.get(fillingText));
